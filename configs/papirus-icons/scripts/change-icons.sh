...
# Remove old generic symlinks so papirus-folders can recreate them for the new color
# (papirus-folders skips symlinks, so stale symlinks from previous colors persist)
# Generic symlinks are simple names like folder-videos.svg (no color prefix)
# Color-specific symlinks like folder-blue-videos.svg should NOT be deleted
SIZES=("22x22" "24x24" "32x32" "48x48" "64x64")
SIZES_WITH_2X=("22x22" "24x24" "32x32" "48x48" "64x64" "22x22@2x" "24x24@2x" "32x32@2x" "48x48@2x" "64x64@2x")
ICONS_DIR="$HOME/.local/share/icons/Papirus-Dark"
# Pattern: matches folder-<variant>.svg or user-<variant>.svg where <variant> has NO hyphens
# This catches: folder-videos.svg, folder-documents.svg, folder-publicshare.svg, etc.
# But NOT: folder-blue-videos.svg, folder-mocha-blue-videos.svg (these have extra hyphens)
for size in "${SIZES[@]}"; do
    places_dir="$ICONS_DIR/$size/places"
    if [ -d "$places_dir" ]; then
        find "$places_dir" -maxdepth 1 -type l \( -name 'folder-[^-]*.svg' -o -name 'user-[^-]*.svg' \) -delete 2>/dev/null || true
    fi
done

~/.local/share/icons/papirus-folders.sh -C "$closest_color" -t "$ICONS_DIR"

# Fix user-*.svg symlinks that papirus-folders couldn't create for hyprtk colors
# hyprtk-* colors don't have user-<color>-*.svg source files, so user-desktop.svg,
# user-home.svg, user-home-open.svg get deleted but never recreated.
# Fix: strip "hyprtk-" prefix and point to the base color's user-*.svg files.
if [[ "$closest_color" == hyprtk-* ]]; then
    base_color="${closest_color#hyprtk-}"
    for size in "${SIZES_WITH_2X[@]}"; do
        places_dir="$ICONS_DIR/$size/places"
        [ -d "$places_dir" ] || continue
        for variant in desktop home home-open; do
            symlink="$places_dir/user-${variant}.svg"
            target="user-${base_color}-${variant}.svg"
            if [ -f "$places_dir/$target" ]; then
                # Create if missing, or update if pointing to wrong color
                current_target=""
                [ -L "$symlink" ] && current_target="$(readlink "$symlink")"
                if [ "$current_target" != "$target" ]; then
                    ln -sf "$target" "$symlink" 2>/dev/null || true
                fi
            fi
        done
    done
...
# Refresh icon cache so file managers pick up the new colors
gtk-update-icon-cache -qf "$ICONS_DIR" 2>/dev/null || true
gtk-update-icon-cache -qf "$HOME/.local/share/icons/Papirus" 2>/dev/null || true
gtk-update-icon-cache -qf "$HOME/.local/share/icons/Papirus-Light" 2>/dev/null || true
